...
persistent actor AtticusCore {
    ...
    private stable var admin_logs_old: [{action: Text; details: Text; timestamp: Int}] = [];
    private stable var btc_price: Float = 0.0;
    private stable var next_withdrawal_id: Nat = 1;
    private stable var platform_wallet_old: {address: Text; balance: Float; total_deposits: Float; total_withdrawals: Float} = {address = ""; balance = 0.0; total_deposits = 0.0; total_withdrawals = 0.0};
    private stable var positions_stable: [(Nat, Position)] = [];
    private stable var user_address_mapping_stable: [(Principal, Text)] = [];
    private stable var user_transactions: [{id: Text; user: Principal; transaction_type: {#Deposit; #Withdrawal}; amount: Float; deposit_id: ?Text; tx_hash: ?Text; timestamp: Int; status: {#Pending; #Confirmed; #Failed}}] = [];
    ...
    system func preupgrade() {
        ...
        positions := positions_stable;
        users := users_stable;
        
        // Convert old admin logs to text format
        admin_logs := Array.map(admin_logs_old, func(log) = log.action # ": " # log.details);
        
        // Migrate platform wallet (drop address field)
        platform_wallet := {
            balance = platform_wallet_old.balance;
            total_deposits = platform_wallet_old.total_deposits;
            total_withdrawals = platform_wallet_old.total_withdrawals;
        };
    };
